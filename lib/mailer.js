...
/* Throws unless `body` is the relay's own `{"ok":true}`. Anything else with a
   200 — most likely the SPA's index.html served because mail-relay.php is not
   deployed at that path — must not be mistaken for a delivered message. */
function assertRelayBody(body) {
  let parsed;
  try {
    ...
    throw new Error(
      looksLikeHtml
        ? `Mail relay at ${MAIL_RELAY_URL} answered with an HTML page, not JSON — mail-relay.php is probably not deployed at that path and the SPA fallback is answering instead.`
        : `Mail relay at ${MAIL_RELAY_URL} answered with something that is not JSON: ${body.slice(0, 200)}`
    );
  }

  if (parsed?.ok !== true) {
    throw new Error(`Mail relay reported failure: ${parsed?.error || body.slice(0, 200)}`);
  }
  return parsed;
}
...
async function sendViaRelay(message) {
  ...
  /* A 200 is not proof the relay ran. If the PHP file is missing, the SPA
     rewrite on the cPanel host answers with index.html and HTTP 200 — trusting
     the status alone would report every message as sent while silently
     dropping all of them. Insist the body is the relay's own JSON. */
  assertRelayBody(body);
...
export async function verifyMailer() {
  if (usingRelay) {
    ...
    /* Checking the status alone is not enough — see assertRelayBody. */
    assertRelayBody(await res.text());
